...
def scroll_js_to_line(line):
    return (
        """<script>
                console.log("HEY")
                var body = window.parent.document.querySelector(".main");
                var line = window.parent.document.getElementById("line"""
        + str(line)
        + '''");
...
def regular_polygon_coord(center, radius, n):
    return [
        [
            center[0] + radius * math.sin((2 * math.pi / n) * i),
            center[1] + radius * math.cos((2 * math.pi / n) * i),
        ]
        for i in range(n)
    ]
...
def check_src_code_folder():
    ...
    for key, value in path_status.items():
        if value is False:
            st.write(
                Exception(
                    f"The source code file for {os.path.join(home_folder,key)} cannot be found"
                )
            )
...
def get_object_view_data(gpu_filter=None, allowed_ops=[]):
    ...
    for source_dev_id in range(gpu_num):
        owned_objs: List[SnoopieObject] = [
            i
            for i in SnoopieObject.all_objects.values()
        ...
        for owned_obj in owned_objs:
            ...
            for i in range(0, int(id_info.size), step):
                ...
                if hex_addr not in related_dict:
                    continue
...
def main():
    ...
    with top_cols[1]:
        print("ops:", ops)
        ops_to_display = st.multiselect(
            "Operations to display", options=ops, default=ops
        )
    ...
    if graph_shape == GRAPH_SHAPES["polygon"]:
        positions = regular_polygon_coord(
            [int(graph_width / 2), int(graph_height / 2)],
            int(min(graph_width, graph_height) / 2) - margin,
            gpu_num,
        )
    ...
    for i in range(gpu_num):
        ...
        filtered_ops = OpInfoRow.filter_by_device_and_ops(
            ops_to_display, [i], all_devices
        )
    ...
    for i in range(gpu_num):
        if chosen_id_tab == str(i):
            st.markdown(
                f"### Objects owned by **<span style='color:{pal[i]}'>GPU{i}</span>**",
                unsafe_allow_html=True,
            )
            ...
            with objects_owned_cols[0]:
                filter_chooser = st.multiselect(
                    "Filter accesses by GPU",
                    other_gpus_selector,
                    default=other_gpus_selector,
                )

            object_view = get_object_view_data(
                [int(i[3:]) for i in filter_chooser], allowed_ops=ops_to_display
            )
            if len(object_view[i]) <= 0:
                ...
                obj_fig = make_subplots(
                    rows=len(object_view[i]),
                    shared_xaxes=True,
                    cols=1,
                ...
                for key in object_view[i].keys():
                    ...
                    if index > 1:
                        ...
                        obj_fig["layout"]["yaxis" + str(index)].update(
                            dict(tickvals=[0], ticktext=[str(key) + ""])
                        )
                    else:
                        obj_fig["layout"]["yaxis" + str(index)].update(
                            dict(tickvals=[0], ticktext=[str(key) + ""])
                    ...
                    obj_fig.update_layout(
                        coloraxis_colorbar=dict(title="Data transfer<br>count")
                    )
                    index += 1
                ...
                obj_fig.update_layout(
                    coloraxis=dict(colorscale=colorscale), showlegend=False
                )
                obj_fig.update_layout(
                ...
                obj_fig["layout"]["xaxis" + str(index - 1)].update(
                    dict(title="Offset", title_standoff=8)
                )
                ...
                def calc_dim_y():
                    st.session_state.ydim = int(
                        len(object_view[i][obj_option][1][0]) / st.session_state.xdim
                    )
                ...
                with obj_2d_cols[0]:
                    obj_option = st.selectbox(
                        "Choose an object to view in 2D", obj_names
                    )
                ...
                with obj_2d_cols[1]:
                    xdim = st.number_input(
                        "X-dimension", min_value=1, on_change=calc_dim_y(), key="xdim"
                    )
                    ...
                    if obj_option is not None:
                        ...
                        for item in obj_data[1][0]:
                            if ind_x == 0:
                                ...
                                reshaped_details.append(
                                    obj_data[0][0][ind_y * xdim : (ind_y * xdim) + xdim]
                                )
            ...
            st.markdown("""---""")
            st.markdown(
                f"### Communication issued by **<span style='color:{pal[i]}'>GPU{i}</span>**",
                unsafe_allow_html=True,
            )
            ...
            to_filter = OpInfoRow.filter_by_device_and_ops(
                allowed_ops=ops_to_display,
                allowed_mem_devs=[i for i in range(gpu_num)],
                allowed_running_devs=[i],
            )
            ...
            for peer_gpu in other_gpus:
                ops_accessed: List[OpInfoRowCombined] = [
                    op for op in to_filter if op.key.mem_dev_id == peer_gpu
                ]
                # st.json([                {
                #     "op_code": i.op_code,
                ...
                for _op in ops_accessed:
                    ...
                    by_lines[line_display_str] = (
                        by_lines[line_display_str] + _op.value.count
                    )
                ...
                for key, value in by_var_name.items():
                    table_objs.append(
                        (
                            key.obj_id_info.obj_id,
                            key.obj_name_info.var_name,
                            value,
                            peer_gpu,
                        )
                    )
            ...
            if len(table_objs) > 0:
                ...
                table_height = min(
                    MIN_TABLE_HEIGHT + len(df) * (ROW_HEIGHT), MAX_TABLE_HEIGHT
                )
                gb = GridOptionsBuilder.from_dataframe(df)
                gb.configure_selection(
                    "single", use_checkbox=False, pre_selected_rows=None
                )
                ...
                with cols[0]:
                    ...
                    if (len(selected_rows) > 0) and selected_rows[0][
                        "_selectedRowNodeInfo"
                    ]["nodeRowIndex"] > 0:
                        st.session_state.object_table_selected = (
                            selected_rows[0]["_selectedRowNodeInfo"]["nodeRowIndex"] - 1
                        )
                ...
                with cols[2]:
                    ...
                    if len(selected_rows) > 0:
                        if selected_rows[0]["code line"] != "total":
                            chosen_file, chosen_line = selected_rows[0][
                                "code line"
                            ].split(":")
                            chosen_line = int(chosen_line.strip()[4:])
                            chosen_file = os.path.join(
                                CodeLineInfoRow.inferred_home_dir, chosen_file.strip()
                            )
                            tkey = "table_select" + str(peer_gpu)
                            file_to_vis = chosen_file
                            if (
                                tkey not in st.session_state
                                or st.session_state[tkey] != chosen_line
                            ):
                ...
                if st.session_state.object_table_selected != None:
                    if not (
                        len(by_var_name_onclick)
                        <= st.session_state.object_table_selected
                        or st.session_state.object_table_selected < 0
                    ):
                        tempop: LineInfo = by_var_name_onclick[
                            st.session_state.object_table_selected
                        ]
                        st.markdown(
                            f"##### Selected Object: {tempop.obj_id_info.obj_id}-{tempop.obj_name_info.var_name}",
                            unsafe_allow_html=True,
                        )
                        tempdict = [
                            (i.file_name, i.func_name, i.line_no)
                            for i in tempop.call_stack
                        ]
                        tempdict.reverse()
...
def show_sidebar():
    ...
    if file_to_vis not in ops_by_line_info["by_file_linenum"]:
        return
    linenum = chosen_line
    if linenum in ops_by_line_info["by_file_linenum"][file_to_vis]:
        with st.sidebar:
            linedata: List[LineInfo] = ops_by_line_info["by_file_linenum"][file_to_vis][
                chosen_line
            ]
            st.markdown(
                f"""
            ...
            st.markdown(
                """<head><script>hljs.highlightAll();</script></head><body>
                            <code class='hljs language-c'>"""
                + src_lines[file_to_vis][linenum - 1]
                + """</code></body>""",
                unsafe_allow_html=True,
            )
            ...
            for i in range(gpu_num):
                ...
                for j in range(gpu_num):
                    ...
                    filtered_ops = [
                        OpInfoRowCombined(p, v)
                        for p, v in OpInfoRow.table().items()
            ...
            df = pd.DataFrame(
                widths, columns=cols_rows_name, index=cols_rows_name
            ).astype("int")
            fig = px.imshow(
                df,
                color_continuous_scale=colorscale,
...
def choose_src_code_file():
    ...
    folder_icon, file_icon = (
        "https://icons.getbootstrap.com/assets/icons/folder.svg",
        "https://icons.getbootstrap.com/assets/icons/file-earmark.svg",
    )
    img_size = 32
    ...
    while totallen > 0:
        ...
        for i in range(item_per_line):
            ...
            fake_item = os.path.join(
                CodeLineInfoRow.inferred_home_dir,
                current_folder[len(home_folder) + 1 :],
                name,
            )
            ...
            with cols[i]:
                if name == "..":
                    ...
                    clicked = clickable_images(
                        [folder_icon],
                        titles=[name],
                        img_style={"width": f"{img_size}px", "height": f"{img_size}px"},
                    )
                    st.write(name)
                    if clicked > -1:
                        st.session_state.current_folder, _ = os.path.split(
                            current_folder
                        )
                        setup_globals()
                    ...
                    st.write(name)
                    if clicked > -1:
                        st.session_state.current_folder = os.path.join(
                            current_folder, name
                        )
                        setup_globals()
                        choose_src_code_file_list = {}
                    ...
                    clicked = clickable_images(
                        ...
                        titles=[name],
                        img_style={"width": f"{img_size}px", "height": f"{img_size}px"},
                        div_style={"asd": f"{choose_src_code_file_list[idx]}"},
                    )
                    ...
                    access_count = (
                        0
                        if fake_item not in ops_by_line_info["by_file"]
                        else ops_by_line_info["by_file"][fake_item]
                    )
                    if ops_by_line_info["total"] == 0:
                        ops_by_line_info["total"] = 1

                    st.write(
                        f"Accesses: {access_count}, %{100 * access_count /ops_by_line_info['total']}"
                    )
...
def show_code():
    ...
    content_head = (
        ...
        + (
            f"<script>{electron_checker.electron_load_highlight()}</script>"
            if electron_checker.is_electron
            else """
    ...
    if (
        file_to_vis in ops_by_line_info["by_file"]
        and file_to_vis in ops_by_line_info["by_file_linenum"]
    ):
        for line_index, linedata in ops_by_line_info["by_file_linenum"][
            file_to_vis
        ].items():
            ftotal = len(linedata)
    ...
    pal = sns.color_palette(
        [scale_lightness(color, 1.2) for color in pal_base_lines]
    ).as_hex()
    pal.reverse()
    ...
    for line in src_lines[file_to_vis]:
        ...
        if comm_percentage == 0:
            str_percentage = " " * 6
        div_inject += (
            "<span style='position:absolute; bottom:0.2rem;'>"
            + str_percentage
            + "</span>"
        )
        content_head += (
            ...
            + """<a 
                        id='line"""
            + str(line_index)
            + """' style='display:inline-block;margin-left:2.5rem'><code class='hljs language-c' style='position:relative;width:100%;
        ...
        content_head += (
            str(line_index)
            + "."
            + "".join([" " * (index_chars - len(str(line_index)))])
            + line
            + "</a></code></div>"
        )
...
if __name__ == "__main__":
    ...
    if not st.session_state.show_filepicker:
        ...
        if start_newfile:
            ...
            if "pickle_filename" in st.session_state and os.path.exists(
                st.session_state.pickle_filename
            ):
