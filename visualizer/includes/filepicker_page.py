...
def folder_choose_dialog():
    ...
    p = subprocess.Popen(["python3", "tk_folder_chooser.py"], cwd=path, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def filepicker_page():
    ...
    choose_home_folder_btn = st.button("Choose source code folder", help="Do not use this button if the profiler is running on a remote machine. Fill the folder path manually instead.")
    ...
    _home_folder = st.text_input("Home folder of source code:", value="/" if _home_folder is None else _home_folder)
    ...
    if _logfile != None and filepicker_button == 1:
