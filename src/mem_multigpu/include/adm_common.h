...
ADM_VISIBILITY
std::streamoff adm_conf_line(const char *var, char *line,
...
static inline void dbgp(const M m, V... v) {
  std::cerr << m;
  dbgp(v...);
};
template <typename M> static inline void dbgp(const M m) {
  std::cerr << m << std::endl;
};
...
ADM_VISIBILITY extern void *(*mmap64_ptr)(void *, size_t, int, int, int,
                                          off64_t);
