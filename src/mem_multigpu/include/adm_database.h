...
enum meta_t {
  ...
  ADM_META_OBJ_TYPE = 1,
  ADM_META_BIN_TYPE = 2,
  ADM_META_STACK_TYPE = 3,
  ADM_MAX_META_TYPES = 4
};
...
public:
  ...
  adm_range_t(uint64_t address1, uint64_t size1, int object_id1, int device_id1)
      : size(size1), address(address1), object_id(object_id1),
        device_id(device_id1) {}

  adm_range_t()
      : size(0), address(0), allocation_pc(0), index_in_object(999),
        device_id(0) {}
  ...
  state_t get_state() const noexcept {
    return static_cast<state_t>(size >> 60);
  };

  void set_state(const state_t state) {
    size = (size & 0x0FFFFFFFFFFFFFFF) | (static_cast<uint64_t>(state) << 60);
  };

  void add_state(const state_t state) {
    size |= (static_cast<uint64_t>(state) << 60);
  };
...
adm_range_t *adm_range_insert(const uint64_t address, const uint64_t size,
                              const uint64_t allocation_pc, const int dev_id,
                              std::string var_name,
                              const state_t state = ADM_STATE_STATIC) noexcept;
...
public:
  ...
  void set_estimated_status(const short estimated_status) noexcept {
    estimated = estimated_status;
  };
  void print(std::ofstream &codeline_outfile) const noexcept;
};

adm_line_location_t *
adm_line_location_insert(const int global_index, std::string file_name,
                         std::string dir_name, std::string sass,
                         const uint32_t line_num, short estimated) noexcept;
...
public:
  allocation_site_t(uint64_t input_pc)
      : object_id(0), pc(input_pc), first_child(NULL), next_sibling(NULL),
        parent(NULL) {}
  ~allocation_site_t() {
    delete next_sibling;
    delete first_child;
  ...
  allocation_site_t *get_first_child() { return first_child; }
  allocation_site_t *get_next_sibling() { return next_sibling; }
  allocation_site_t *get_parent() { return parent; }
  void set_object_id(int obj_id) { object_id = obj_id; }
  void set_pc(uint64_t pc1) { pc = pc1; }
  void set_first_child(allocation_site_t *child) { first_child = child; }
  void set_next_sibling(allocation_site_t *sibling) { next_sibling = sibling; }
...
public:
  ...
  adm_object_t()
      : object_id(0), allocation_site(NULL), allocation_pc(0), line_num(0),
        data_type_size(0), range_count(0) {}

  adm_object_t(int object_id1, allocation_site_t *allocation_site1,
               uint32_t data_type_size1)
      : object_id(object_id1), allocation_site(allocation_site1),
        data_type_size(data_type_size1), range_count(0) {}

  int get_object_id() const noexcept { return object_id; };

  void set_object_id(const int obj_id) noexcept { object_id = obj_id; };

  allocation_site_t *get_allocation_site() const noexcept {
  ...
  void set_data_type_size(const int type_size) noexcept {
...
adm_object_t *
adm_object_insert(const uint64_t allocation_pc, std::string varname,
                  const uint32_t element_size, std::string filename,
...
public:
  allocation_line_t(uint64_t pc1, std::string func_name1,
                    std::string file_name1, uint32_t line_num1)
      : pc(pc1), func_name(func_name1), file_name(file_name1),
        line_num(line_num1) {}
  uint64_t get_pc() { return pc; };
  ...
  void print(std::ofstream &object_outfile) {
    object_outfile << pc << ",\"" << func_name << "\"," << file_name << ","
                   << line_num << "\n";
  }
};
// after
