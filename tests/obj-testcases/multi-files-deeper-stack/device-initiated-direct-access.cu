...
extern cudaError_t foo_hostalloc(int **buf, size_t size);
...
int main(int argc, char* argv[]) {
  ...
  int *g0 = NULL;
  cudaSetDevice(gpuid[0]);
  gpuErrchk(foo_alloc(&g0, buf_size));

  int *g1 = NULL;

  cudaSetDevice(gpuid[1]);
  gpuErrchk(foo_alloc(&g1, buf_size));
  ...
  cudaSetDevice(gpuid[0]);
  int *h0 = NULL;
  gpuErrchk(foo_hostalloc(&h0, buf_size));

  cudaSetDevice(gpuid[1]);
  int *h1 = NULL;
  gpuErrchk(foo_hostalloc(&h1, buf_size));
