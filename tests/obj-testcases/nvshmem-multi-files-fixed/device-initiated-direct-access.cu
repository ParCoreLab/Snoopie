...
int main(int argc, char* argv[]) {
  ...
  fprintf(stderr, "before the nvshmem_malloc of g0\n");
  //int *g0 = (int *) nvshmem_mallocWRAP (buf_size * sizeof(int), "g0", sizeof(int));
  //fprintf(stderr, "before the nvshmem_malloc of g0\n");
  int *g0 = (int *) nvshmem_malloc (buf_size * sizeof(int));
  fprintf(stderr, "g0's offset: %lx, size: %ld\n", (long unsigned int) g0, buf_size);
	//(real*)nvshmem_mallocWRAP(
        //nx * (chunk_size_high + 2) *
        //sizeof(real), "a", sizeof(real)); 

  //int *g1 = NULL;
  //gpuErrchk(cudaMalloc(&g1, buf_size));
  fprintf(stderr, "before the nvshmem_malloc of g1\n");
  //int *g1 = (int *) nvshmem_mallocWRAP (buf_size * sizeof(int), "g1", sizeof(int));
  //fprintf(stderr, "before the nvshmem_malloc of g1\n");
  int *g1 = (int *) nvshmem_malloc (buf_size * sizeof(int));
  fprintf(stderr, "g1's offset: %lx, size: %ld\n", (long unsigned int) g1, buf_size);

  //int *g2 = NULL;
  //gpuErrchk(cudaMalloc(&g2, buf_size));
  fprintf(stderr, "before the nvshmem_malloc of g2\n");
  //int *g2 = (int *) nvshmem_mallocWRAP (buf_size * sizeof(int), "g2", sizeof(int));
  //fprintf(stderr, "before the nvshmem_malloc of g2\n");
  int *g2 = (int *) nvshmem_malloc (buf_size * sizeof(int));
  fprintf(stderr, "g2's offset: %lx, size: %ld\n", (long unsigned int) g2, buf_size);
