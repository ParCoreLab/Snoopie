...
__global__ void __launch_bounds__(1024, 1)
    jacobi_kernel(real *a_new, real *a, const int iy_start, const int iy_end,
                  const int nx, const int grid_dim_x, const int iter_max,
  ...
  const int comp_start_iy =
      ((blockIdx.x / grid_dim_x) * blockDim.y + threadIdx.y + iy_start + 1) *
      nx;
  const int comp_start_ix =
      ((blockIdx.x % grid_dim_x) * blockDim.x + threadIdx.x + 1);
  ...
  while (iter < iter_max) {
    if (blockIdx.x == gridDim.x - 1) {
      ...
      if (!cta.thread_rank()) {
        remote_am_done_writing_to_top_neighbor[next_iter_mod * 2 + 1] =
            iter + 1;
      }
      ...
      for (int iy = comp_start_iy; iy < end_iy; iy += comp_size_iy) {
        for (int ix = comp_start_ix; ix < end_ix; ix += comp_size_ix) {
          a_new[iy + ix] = 0.25 * (a[iy + ix + 1] + a[iy + ix - 1] +
...
double single_gpu(const int nx, const int ny, const int iter_max,
                  ...
                  decltype(jacobi_kernel_single_gpu) kernel) {
  real *a;
  real *a_new;
  ...
  dim3 dim_grid((nx + dim_block_x - 1) / dim_block_x,
                (ny + dim_block_y - 1) / dim_block_y, 1);
  ...
  while (iter < iter_max) {
    ...
    //        calculate_norm = (iter % nccheck) == 0 || (print && ((iter % 100)
    //        == 0));
    ...
    CUDA_RT_CALL(cudaMemcpyAsync(a_new, a_new + (iy_end - 1) * nx,
...
void report_results(const int ny, const int nx, real *a_ref_h, real *a_h,
  ...
  if (result_correct) {
    ...
    if (compare_to_single_gpu) {
      printf("Non-persistent kernel - %dx%d: 1 GPU: %8.4f s, %d GPUs: %8.4f s, "
             "speedup: "
             "%8.2f, "
             "efficiency: %8.2f \n",
...
#pragma omp critical
    ...
    if (compare_to_single_gpu && 0 == dev_id) {
      ...
      runtime_serial_non_persistent =
          single_gpu(nx, ny, iter_max, a_ref_h, 0, true);
    }
...
#pragma omp barrier
    ...
    if (top != dev_id) {
      ...
      if (canAccessPeer) {
        ...
        std::cerr << "P2P access required from " << dev_id << " to " << top
                  << std::endl;
      }
      if (top != bottom) {
        canAccessPeer = 0;
...
#pragma omp barrier
    ...
    CUDA_RT_CALL(cudaMalloc(halo_buffer_for_top_neighbor + dev_id,
                            2 * nx * sizeof(real)));
    CUDA_RT_CALL(cudaMalloc(halo_buffer_for_bottom_neighbor + dev_id,
                            2 * nx * sizeof(real)));

    CUDA_RT_CALL(cudaMemset(halo_buffer_for_top_neighbor[dev_id], 0,
                            2 * nx * sizeof(real)));
    CUDA_RT_CALL(cudaMemset(halo_buffer_for_bottom_neighbor[dev_id], 0,
                            2 * nx * sizeof(real)));

    CUDA_RT_CALL(cudaMalloc(is_top_done_computing_flags + dev_id,
                            num_flags * sizeof(int)));
    ...
    if (dev_id < num_ranks_low) {
      ...
      iy_start_global = num_ranks_low * chunk_size_low +
                        (dev_id - num_ranks_low) * chunk_size_high + 1;
    }
    int iy_end_global =
    ...
    initialize_boundaries<<<(ny / num_devices) / 128 + 1, 128>>>(
        a[dev_id], a_new[dev_id], PI, iy_start_global - 1, nx, chunk_size + 2,
        ny);
    CUDA_RT_CALL(cudaGetLastError());
    ...
    CUDA_RT_CALL(cudaMemcpy((void *)halo_buffer_for_bottom_neighbor[dev_id],
                            a[dev_id], nx * sizeof(real),
                            cudaMemcpyDeviceToDevice));
...
#pragma omp barrier
    ...
    CUDA_RT_CALL(cudaLaunchCooperativeKernel(
        (void *)SSMultiThreadedTwoBlockComm::jacobi_kernel, dim_grid, dim_block,
...
#pragma omp master
    {
      report_results(ny, nx, a_ref_h, a_h, num_devices,
                     runtime_serial_non_persistent, start, stop,
