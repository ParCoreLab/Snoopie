...
int run_bfs(bfs_func func, graph_t *graph, int start_vertex, int *expected,
  ...
  for (int i = 0; i < runs; i++) {
    ...
    int next_time =
        func(graph->v_adj_list, graph->v_adj_begin, graph->v_adj_length,
             graph->num_vertices, graph->num_edges, start_vertex, result);
...
const bfs_func bfs_functions[] = {
    ...
    //&single_cg_frontier,
    //&multi_split_queue,
    &bfs_cuda_simple, &bfs_cuda_frontier, &bfs_cuda_frontier_numbers,
...
void run_all_bfs(graph_t *graph, int start_vertex) {
  ...
  for (int i = 0; i < num_bfs; i++) {
    ...
    if (verbose) {
      if (!wrong_result[i]) {
        printf("%s,%s,%i,%i,%i,%f\n", filename, bfs_names[i].c_str(), N_GPU,
               BLOCKS, THREADS, avg_runtime);
      } else {
        printf("%s,%s,%i,%i,%i,-1\n", filename, bfs_names[i].c_str(), N_GPU,
               BLOCKS, THREADS);
      }
...
int *read_part(graph_t *graph) {
  ...
  ifstream infile(string(filename).substr(0, string(filename).length() - 4) +
                  ".g.part." + to_string(N_GPU));
  if (!infile.is_open()) {
    ...
    while (infile >> owner) {
      if (owner == device) {
        ...
        for (int i = graph_orig->v_adj_begin[vertex];
             i < graph_orig->v_adj_begin[vertex] + v_adj_length[vertex_index];
             i++) {
...
graph_t *load_graph() {
  ...
  if (verbose)
    printf("avg_deg=%f deg_stddev=%f median=%i\n", avg_degree, degree_stddev,
           degs[num_vertices / 2]);
...
graph_t *load_graph_double() {
  ...
  for (int i = 0; i < vertices_read; i++) {
    ...
    for (int j = 0; j < list_size; j++) {
      v_adj_lists[vertices_read + i + 1].push_back(v_adj_lists[i][j] +
                                                   vertices_read + 1);
    }
  }
  ...
  for (int j = 0; j < rand_list_size; j++) {
    if (verbose)
      printf("%d ", v_adj_lists[rand_vert][j]);
  }
  ...
  for (int j = 0; j < rand_list_size; j++) {
    if (verbose)
      printf("%d ", v_adj_lists[rand_vert + vertices_read + 1][j]);
  }
...
int main(int argc, char *argv[]) {
  if (argc < 5) {
    printf("Usage: %s -f filename(str) -n num_gpu(int) -b blocks(int) -t "
           "threads(int)\n",
           argv[0]);
    printf("Optional: -s start_vertex_id(int) -r runs(int) -m method(int) -l "
           "fma_load(int) ");
    printf("-o offload_threshold(float) -c communication_threshold(int) -v "
           "verbose(bool)\n");
  ...
  while ((opt = getopt(argc, argv, "f:b:t:s:r:n:m:l:o:c:vp")) != -1) {
    ...
    case 'm':
      ...
      break;
    case 'o':
      OFF_THRESH = atof(optarg);
      break;
    case 'c':
      COMM_THRESH = atof(optarg);
      break;
    case 'v':
      verbose = true;
      break;
  ...
  if (METHOD == -2) {
    ...
    for (int vertex = range_from; vertex < range_to; vertex += stride) {
      ...
      if (METHOD >= 0) {
        int next_runtime =
            run_bfs(bfs_functions[METHOD], graph, vertex, expected, runs);
        // printf("GPU_all=%d\n",
        // std::chrono::duration_cast<std::chrono::milliseconds>(t2-t1).count());
        // printf("%s,%s,%i,%i,%i,%d\n", filename, bfs_names[METHOD].c_str(),
        // N_GPU, BLOCKS, THREADS, next_runtime);
      }
      delete[] expected;
    }
  }
